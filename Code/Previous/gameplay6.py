...
class Deck(object):
    ...
    def dealPlayer(self, amount, player):
        ...
        if not clnt.clientExists() or srvr.serverExists(): #either not a client (singleplayer) or the server
            cardData = {'Hand':[], 'name':player.name} #dictionary for server (if needed)
            for i in range(0,amount):
                ...
                cardData["Hand"].append([self.cards[index].value, self.cards[index].suit])
    ...
    def dealCommunity(self):
        ...
        if not clnt.clientExists() or srvr.serverExists(): #either not a client (singleplayer) or the server
            ...
            for i in range(0, amount):
                ...
                cardData['Community'].append([self.cards[index].value, self.cards[index].suit])
                self.used.append(self.cards.pop(index)) #removing card and adding to used
    ...
    def clientDealPlayer(self, allCards, name):
        ...
        for card in toBeRemoved: #removing cards
            self.used.append(self.cards.remove(card)) #adding to used
...
class Tutor(object):
    ...
    def refresh(self, event):
        if ROUND.stage == "Turn" or ROUND.stage == "Flop" or ROUND.stage == "River":
            ...
            self.EV, self.potOdds = self.calcEV(ROUND.pot, self.equity, amCall)
...
def decodeMessage(message):
    ...
    for each in message:
        if each == "allPlayers": #new client!
            ...
            allPlayers.append(multiPlayer(name, position, index, COLOURS[index], allCoOrds[index][0], allCoOrds[index][1]))
            allPlayers[-1].createBlock()
            nextCreateIndex += 1
        elif each == "Move":
            name = message[each]['name']
            ...
            name = message['name']
            allCards = message[each]
            DECK.clientDealPlayer(allCards, name)
        elif each == "Start":
            startMulti(None)

def startMulti(event):
    global shortagePlayersPopUp, leaveShortageBtn, allPlayers
    if len(allPlayers) >= 2:
