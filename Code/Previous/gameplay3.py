...
class Player(object):
    ...
    def currTurn(self):
        ...
        if not ROUND.timerHide: #if ingame settings is open
            self.createTimer()
...
class Round(object):
    ...
    def showdown(self): #TODO multiple pots
        winnerIndex = self.calcWinner()
        if winnerIndex == -1: #someone is all-in so pots need to split
            #TODO pot splitter
            pass
        elif winnerIndex.isinstance(int): #single winner
        ...
        self.winnerBanner = canvas.create_image(300, 300, image=self.bannerImage, anchor=tk.NW, tags="banner")
...
def ingameSettings(event):
    global inGameBgImage, inGameCanvas, exitButton, saveButton
    buttonPressSound.play()
    ...
    saveButton = newButton(inGameCanvas, 1300, 600, "tealButton.fw.png", "inGame", saveGame)
    saveButton.addLabel(inGameCanvas, "SAVE", "white", (1440, 650), 30, "inGame", saveGame)
