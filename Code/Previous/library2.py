...
class newPopUp(object):
    def __init__(self, allText, func, scale, coOrds):
        ...
        self.popUpLabel = canvas.create_image(400, 400, image=popUpImage, anchor=tk.NW, tags="popUp") #creating gery pop up box
        #text to inform user
        self.text = canvas.create_text(875, 500, text=allText, font = (FONTNAME,40), fill = "white", tags="popUp", justify=CENTER)

        if scale is not None: #if there is a scale to remove
    ...
    def closePopUp(self, scale, coOrds, event):
        ...
        canvas.delete("popUp") #deleting pop up entities
        if scale is not None: #replacing the scale
