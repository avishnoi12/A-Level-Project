...
NAMES = ["Gareth","Sue","Steve","Iain","Vin","Wendy","Victoria","James","Paul","Caroline"]
SUITS = ["C","D","H","S"]
...
class Player(object):
    def __init__(self, name, position, index, colour, x, y):
        ...
        #*creating icon co ords
        if self.y > 515:
            self.icony = self.y - 50
        elif self.y < 515:
            self.icony = self.y + 170
        else:
            self.icony = self.y

        if self.x > 960:
            self.iconx = self.x-140
            xCoOrd = self.iconx - 80
        elif self.x < 960:
            self.iconx = self.x+70
            xCoOrd = self.iconx + 60
        else:
            self.iconx = self.x - 30
            xCoOrd = self.iconx + 65
            if self.y == 70:
                self.icony += 50
            elif self.y == 680:
                self.icony -= 30

        self.iconLabel = canvas.create_text(xCoOrd,self.icony, font=(BOLDFONT, 30), anchor=tk.NW)
    ...
    def endTurn(self):
        ...
        self.timerCount = 10 #stops timer update
        self.timer.destroy()
    ...
    def placeIcon(self, imgPath):
        ...
        self.icon = canvas.create_image(self.iconx, self.icony, image=self.iconImage, anchor=tk.NW)
        ...
        elif imgPath == "allIn.fw":
            canvas.itemconfigure(self.iconLabel, text="$"+str(self.currBet), fill="white")
    ...
    def timerUpdate(self):
        timePer = (amountTime*1000)//10
        try:
            self.timer['value'] += 10 #validating timer exists
        except:
            pass #stops the recursive call
        else:
            if self.timerCount < 9: #if timer hasnt reached its limit
                self.timerCount += 1
                canvas.after(timePer, self.timerUpdate)
...
class User(Player):
    def __init__(self, name, position, index, colour, x, y):
        ...
        if getJson("Settings.json", ("settings","Tutor")) == "ON":
            if not clnt.clientExists(): #singleplayer game
    ...
    def currTurn(self):
        ...
        if self.preSelect.get() == 5: #no preselect button selected
            ...
            if ROUND.currBet == 0:
                minRaise = ROUND.bigBlind
            else:
                minRaise = ROUND.currBet*2

            if minRaise <= self.balance:       
                self.raiseSlider = tk.Scale(canvas, from_=self.balance, to=minRaise, font=(FONTNAME,30), bg="green", length=380
                    ,showvalue=0, orient=tk.VERTICAL, command=self.sliderUpdate, troughcolor="#00FF00", width=30, bd=0)
                self.raiseSlider.place(x=100, y=470)
                self.raiseLabel = canvas.create_text(110,875, text="$"+str(self.raiseSlider.get()),fill="#00FF00",font=(FONTNAME,30), tags="actions")

                self.allInButton = newButton(canvas, 90, 400, "Action/allIn.fw.png", "actions", self.allInSlider)

                #*creating the raise buttons
                raiseFunc = partial(self.betRaise, None)
                self.raiseBtn = newButton(canvas, 60, 930, "Action/raise.fw.png", "actions", raiseFunc)
            
            #creating the other action buttons
            ...
            if ROUND.currBet == 0: #only creates check button if possible to check
                self.checkBtn = newButton(canvas, 1300, 930, "Action/check.fw.png", "actions", self.check)

            if self.tutor != "OFF":
                self.tutor.suggestMove()
    ...
    def allInSlider(self, event):
        self.raiseSlider.set(self.balance)
...
class AI(Player):
    ...
    def chooseMove(self):
        ...
        maxTime = (amountTime-1)*1000
        timeToMove = random.randint(1000, maxTime) #how much time taken to move
...
class Round(object):
    def __init__(self):
        ...
        self.potLabel = canvas.create_text(920, 370, text="$"+str(self.pot), font=(FONTNAME,30), fill="#00FFFF", anchor=tk.NW)
    ...
    def nextStage(self):
        ...
        if newRound:
            ...
            allPlayers[self.currPlayer].timerCount = 10 #stopping their timer
            #canvas.after(300, self.nextPlayer, self.currPlayer)
            self.nextPlayer(self.currPlayer)
...
class Tutor(object):
    ...
    def expertMove(self):
        ...
        moveScore += self.EV*3
        if ROUND.stage == "Turn" or ROUND.stage == "Flop" or ROUND.stage == "River":
            moveScore += (RANKS[self.bestRank[1]])*5

            for rank in self.rankProbs:
                rankValue = RANKS[rank]
                moveScore += (rankValue**2)*(self.rankProbs[rank])

            moveScore += self.outs*2
        else:
            moveScore = (self.PFScore + self.equity)*self.potOdds

        if player.position in ["BTN", "CO"]: #late positions
            moveScore + 140
        elif player.position in ["LJ","HJ", "MP+1"]: #middle positions
            moveScore += 50
        elif player.position in ["SB","BB", "UTG"]: #early positions
            moveScore -= 120
        ...
        if move == "FOLD":
            ...
            if moveScore >= 350:
                ...
                move = "FOLD"
        elif move == "MINRAISE" or move == "HALFRAISE":
            if moveScore >= 300:
                move = "ALLIN"
            elif moveScore >= 100:
                moveScore = "HALFRAISE"
...
def ingameSettings(event):
    ...
    global muteButton, fullScreenButton, unMuteButton, gameCodeLabel, AILevelButton, bgColourButtons, bgColourLabel
    ...
    #*Backgorund colour buttons
    bgColourButtons = newRadioButton(inGameCanvas, 700 ,120, ["RED","GREEN","BLUE"],"inGame","Settings.json",("settings","TexturePack"))
    bgColourLabel = newOptionLabel(inGameCanvas, 300, 100, "BG COLOUR:")

    if not clnt.clientExists(): #singleplayer
        ...
        gameCodeLabel = inGameCanvas.create_text(300, 100, text="Game code is: "+str(srvr.HOST), font=FONTNAME, fill="white")
...
def playerInfo():
    ...
    jsonData["Gameplay"]["Time"] = amountTime
...
def exitSettings(event):
    ...
    #*updating background
    bgColour = bgColourButtons.get()
    bgImage = ImageTk.PhotoImage(Image.open("../Entities/Background/Gameplay/"+bgColour+".png")) #path
    canvas.itemconfigure(bg, image=bgImage)
...
def createGameplay(window):
    global canvas, startStack, texturePack, allPlayers, allCoOrds, allPositions, amountTime
    global DECK, ROUND, bgImage, tableImage, deckImage, exitButton, settingsButton, bg, AILEVEL
    ...
    amountTime = settings["options"]["Time"]
    AILEVEL = ALLAILEVELS[settings["options"]["AI Level"]]
...
def decodeMessage(message):
    global THEUSER, maxPlayers, allPlayers, allCoOrds, allPositions, nextCreateIndex, amountTime
    ...
    for each in message:
        if each == "allPlayers": #new client!
            ...
            amountTime = message['Gameplay']['Time']
...
def runMultiGameplay(window):
    global startGameBtn, nextCreateIndex, allPlayers, allPositions, allCoOrds, maxPlayers, amountTime, THEUSER

    createGameplay(window)

    #*getting number of players
    maxPlayers = int(getJson("Settings.json", ("options","MaxPlayer")))
    amountTime = getJson("Settings.json", ("options","Time"))
    if maxPlayers == 9: #changing variable to fit with number of players
        allPositions = MAX9[:]
        allCoOrds = NINECOORDS[:]
    elif maxPlayers == 6:
